...
``OpenAIProvider``: the ``ModelProvider`` adapter for OpenAI's Chat
Completions API (``POST {base_url}``, e.g.
...
logger = get_logger("providers.openai_provider")

_PROVIDER_TYPE = "openai"


class OpenAIProvider:
    """``ModelProvider`` implementation for OpenAI's Chat Completions API."""
    ...
    def generate(self, prompt: str) -> ProviderResponse:
        ...
        try:
            ...
            logger.warning(
                "OpenAI request timed out: model=%s timeout=%.1fs",
                self._config.model,
        ...
        logger.info(
            "OpenAI request succeeded: model=%s duration=%.2fs",
            self._config.model,
            duration,
        )
